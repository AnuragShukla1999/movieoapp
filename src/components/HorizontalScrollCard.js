...
const HorizontalScrollCard = ({ data = [], heading, trending, media_type }) => {
    ...
    return (
        <div className='conatiner mx-auto px-3 my-10'>
            ...
            <div className='relative'>
                <div ref={containerRef} className='grid grid-cols-[repeat(auto-fit,230px)] grid-flow-col gap-6 overflow-hidden overflow-x-scroll relative z-10 scroll-smooth transition-all scrolbar-none' >
                    {
                        data.map((data, index) => {
                            <Card key={data.id+"heading"+index} data={dat} />
                        })
                    }
                </div>
                ...
                <div className='absolute top-0 hidden lg:flex justify-between w-full h-full items-center'>
                    <button onClick={handlePrevios} className='bg-white p-1 text-black rounded-full -ml-2 z-10' >
                        <FaAngleLeft/>
                    </button>
